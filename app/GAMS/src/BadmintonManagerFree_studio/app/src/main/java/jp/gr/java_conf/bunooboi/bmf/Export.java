...
public class Export {
    ...
    public Export(Context context) {
        ...
        sb.append(Calculation.GameStartTime + "," + Calculation.GameEndTime + ",\n");
        sb.append(Calculation.MaxGame + "," + Calculation.MaxPeople + "," + Calculation.MaxEvent + "," +
                Calculation.WinScore + ",\n");
        sb.append(Calculation.TeamColor[0] + "," + Calculation.TeamColor[1] + ",\n");
    }
    ...
    public void Save() {// 書き込み
        try {
            ...
            BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(new FileOutputStream
                    (Calculation.FilePath + Calculation.GameName + ".csv", false), "UTF-8"));
