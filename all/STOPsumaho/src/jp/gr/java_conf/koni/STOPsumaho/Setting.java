...
public class Setting extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		textview5.setText("※BACKキーを設定回数押すことで強制解除することができます。");
		textview5.setTextSize(8 * Main.setScaleSize(getApplicationContext()));
		...
		button1.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				try {
					Main.releasecount = Integer.parseInt(edittext.getText().toString());
					savekaizyocount();
					Toast.makeText(getApplicationContext(), "BACKキー" + String.valueOf(Main.releasecount) + "回", Toast.LENGTH_LONG).show();
	...
	private void savekaizyocount() {
		SharedPreferences sp = PreferenceManager.getDefaultSharedPreferences(getApplicationContext());
		sp.edit().putInt("Savekaizyocount", Main.releasecount).commit();
	}
}
