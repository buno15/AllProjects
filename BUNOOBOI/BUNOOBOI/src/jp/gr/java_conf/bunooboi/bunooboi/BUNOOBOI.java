...
public class BUNOOBOI {
	...
	public BUNOOBOI(KA ka, int kaCount, int ka_rangelimit, int wall_rangelimit, int spray_rangelimit, int touch_rangelimit, int kaclash, int wallclash,
			int sprayclash, int touchclash, int intake) {
		this.ka = ka;
	...
	void SuperEgo() {
		for (int i = 0; i < kaCount; i++) {// 蚊同士の衝突回避
			if (!(other_ka[i].equals(ka)))
				Clash(ka.ka_x, ka.ka_y, ka.ka_w / 2, ka.ka_h / 2, other_ka[i].ka_x, other_ka[i].ka_y, other_ka[i].ka_w / 2, other_ka[i].ka_h / 2, KA_RANGELIMIT,
						KACLASH);
		}
		...
		if (Play.spray != null)
			Clash(ka.ka_x, ka.ka_y, ka.ka_w / 2, ka.ka_h / 2, Play.spray.smoke_x, Play.spray.smoke_y, Play.spray.smoke_w, Play.spray.smoke_h / 2,
					SPRAY_RANGELIMIT, SPRAYCLASH);// スプレー回避
